...
class DefeatRoachesAgent(gym.Env):
    def __init__(self):
        super(DefeatRoachesAgent, self).__init__()
        self.current_timestep = None
        self.cumulative_reward = 0
        self.total_allied_units = 9
        ...
        # Action space: [Select Mask (9 bits), Action Type (Move/Attack), X, Y]
        self.action_space = spaces.MultiDiscrete([2] * 9 + [3, 48, 48])
    ...
    def _calculate_reward(self, timestep):
        """Calcula a recompensa com base nos Roaches derrotados e penaliza unidades aliadas destruídas."""
        ...
        killed_roaches = (current_killed_units - self.cumulative_kills)
        if killed_roaches > 0:
            reward += killed_roaches * 10
    ...
    def _transform_action(self, action, timestep):
        ...
        unit_mask = action[:9]
        action_type, target_x, target_y = action[9:]
