...
class DefeatZerglingsAndBanelingsAgent(gym.Env):
    def __init__(self):
        super(DefeatZerglingsAndBanelingsAgent, self).__init__()
        self.current_timestep = None
        self.cumulative_reward = 0
        self.total_allied_units = 0
        ...
        # Espaço de ações fixo para 20 unidades
        self.action_space = spaces.MultiDiscrete([2] * 50 + [3, 48, 48])
    ...
    def _calculate_reward(self, timestep):
        """Calcula a recompensa com base nos inimigos derrotados, penaliza unidades aliadas destruídas e ociosidade."""
        player_score = timestep.observation['score_cumulative']
        reward = 0

        # Recompensa baseada em inimigos derrotados
        current_killed_units = player_score.score
        killed_enemies = (current_killed_units - self.cumulative_kills)
        if killed_enemies > 0:
            reward += killed_enemies * 50
        ...
        if all(unit.order_length == 0 for unit in current_allied_units):
            reward -= 1
    ...
    def _transform_action(self, action, timestep):
        ...
        unit_mask = action[:50]  # Fixo para 20 unidades
        action_type, target_x, target_y = action[50:]
